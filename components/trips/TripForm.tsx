...
export function TripForm({ initialData, isEdit = false, tripId }: TripFormProps) {
    ...
    const onSubmit = async (data: any) => {
        try {
            ...
            toast.error(error.message || `Failed to ${isEdit ? 'update' : 'create'} trip`)
        }
    }
    ...
    return (
        <div className="max-w-7xl mx-auto">
            {/* Header */}
            <div className="mb-8">
                <Link href={isEdit ? `/trips/${tripId}` : "/trips"} className="inline-flex items-center text-teal-600 hover:text-teal-700 mb-4">
