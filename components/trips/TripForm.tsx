...
import { useCreateTrip, useUpdateTrip } from '@/lib/hooks/api'
...
export function TripForm({ initialData, isEdit = false, tripId }: TripFormProps) {
    ...
    const {
        ...
        defaultValues: {
            packageTypes: initialData?.packageTypes || [],
            restrictions: initialData?.restrictions || [],
            transportMode: initialData?.transportMode || 'car',
            maxWeight: initialData?.maxWeight || undefined,
            maxDimensions: initialData?.maxDimensions || { length: undefined, width: undefined, height: undefined },
            pricePerKg: initialData?.pricePerKg || undefined,
            minimumPrice: initialData?.minimumPrice || undefined,
            maximumPrice: initialData?.maximumPrice || undefined,
            images: initialData?.images || [],
            departureDate: initialData?.departureDate ? new Date(initialData.departureDate).toISOString().slice(0, 16) : '',
            arrivalDate: initialData?.arrivalDate ? new Date(initialData.arrivalDate).toISOString().slice(0, 16) : '',
    ...
    useEffect(() => {
        if (isEdit && initialData) {
            ...
            reset({
                ...
                maxDimensions: initialData.maxDimensions || { length: '', width: '', height: '' },
    ...
    const onSubmit = async (data: any) => {
        console.log('Submitting trip data:', tripId)
        try {
            const tripData = {
                ...data,
            ...
            if (isEdit && tripId) {
                await updateTrip.mutateAsync({ id: tripId, data: tripData })
                toast.success('Trip updated successfully!', toastSuccessStyle)
                router.push(`/trips/${tripId}`)
            } else {
                const result = await createTrip.mutateAsync(tripData)
                toast.success('Trip posted successfully!', toastSuccessStyle)
                router.push(`/trips/${result.id}`)
            }
        } catch (error: any) {
            toast.error(error.message || `Failed to ${isEdit ? 'update' : 'create'} trip`, toastErrorStyle)
        }
    }

    const nextStep = () => {
        console.log('Submitting trip data:', tripId, isEdit)
        ...
        <>
            ...
            <div className="max-w-7xl mx-auto mb-28">
                ...
                <div className="mb-8">
                    ...
                    <h1 className="text-3xl font-bold text-gray-900">{isEdit ? 'Edit Trip' : 'Post a Trip'}</h1>
                    <p className="mt-2 text-gray-600">
                        {isEdit ? 'Update your travel plans below' : 'Share your travel plans and earn money by delivering packages'}
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                                        ...
                                        {packageTypeOptions.map((type) => (
                                            <label key={type} className="flex items-center">
                                                <input
                                                    ...
                                                    className="rounded border-gray-300 text-teal-600 focus:ring-teal-500 accent-teal-600"
                                                />
                    ...
                    <div className="flex justify-between">
                        ...
                        {step < 4 ? (
                            ...
                            >
                                {isSubmitting ? (isEdit ? 'Updating Trip...' : 'Posting Trip...') : (isEdit ? 'Update Trip' : 'Post Trip')}
                            </Button>
                        )}
                    </div>
                </form>
